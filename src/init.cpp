...
// Copyright (c) 2021 The vegancoin Core developers
 ...
 */
static const char* BITCOIN_PID_FILENAME = "vegancoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/martinvrx/vegancoin>";
    const std::string URL_WEBSITE = "<https://www.vegancoin.me>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("vegancoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if vegancoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if vegancoin is started while in a temporary directory.\n",
